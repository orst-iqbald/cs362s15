...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
	  ...
	  res = pAdventurer(state, handPos);
	  
      return res;
			
    case council_room:
      
	  res = pCouncilRoom(state, handPos);
	  
      return res;
			
    case feast:
 
      	res = pFeast(state, choice1, handPos);
    ...
    case minion:
		...
		res = pMinion(state, choice1, choice2, handPos);
...
int pCouncilRoom(struct gameState *state, int handPos){
...
int pFeast(struct gameState *state, int choice1, int handPos){
    ...
    while(x == 1) { //Buy one card
		if (supplyCount(choice1, state) <= 0){
			...
			discardCard(handPos, currentPlayer, state, 0);  //Trash the feast card.
...
int pAdventurer(struct gameState *state, int handPos){
	...
	discardCard(handPos, currentPlayer, state, 1);  //Discard the Adventurer card
    return 0;
...
int pMinion(struct gameState *state, int choice1, int choice2, int handPos){
