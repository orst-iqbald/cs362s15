...
void newAdventurer(int currentPlayer, int handPos, struct gameState* state) {
		    int drawntreasure = 0;
...
void newCouncilRoom(int currentPlayer, int handPos,
		struct gameState* state) {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    {
    case adventurer:
      newAdventurer(currentPlayer,handPos, state);
      return 0;

    case council_room:

	  newCouncilRoom(currentPlayer, handPos, state);
      return 0;
    ...
    case smithy:
      newSmithy(currentPlayer, handPos, state);
      return 0;

    case village:
      newVillage(currentPlayer, handPos, state);
      return 0;

    case baron:
	  newBaron(choice1, currentPlayer, state);
      return 0;
