...
int playAdventurer(struct gameState *state, int cur, int* temp, int drawnTreasure, int card, int z){
	while(drawnTreasure<2){
	if (state->deckCount[cur] <1){//if the deck is empty we need to shuffle discard and add to deck
	  if(shuffle(cur, state) < 0)
		  return -1;
	}
	if (drawCard(cur, state) < 0)
		return -1;
	card = state->hand[cur][state->handCount[cur]-1];//top card of hand is most recently drawn card.
	if (card == copper || card == silver || card == gold)
	  drawnTreasure++;
	else{
	  temp[z]=card;
	  state->handCount[cur]--; //this should just remove the top card (the most recently drawn one).
	  z++;
	}
      }
      while(z>=0){
	state->discard[cur][state->discardCount[cur]++]=temp[z]; // discard all cards in play that have been drawn
	z=z-1;
      }
		temp = NULL;
		free(temp);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
	 	playAdventurer(state, currentPlayer, temphand, drawntreasure, cardDrawn, z);
      /*while(drawntreasure<2){
