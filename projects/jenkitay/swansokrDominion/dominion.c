...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
     return play_adventurer(state, currentPlayer, handPos, temphand);
    ...
    case remodel:
      return play_remodel(state, currentPlayer, handPos, choice1, choice2);
    ...
    case cutpurse:
      return play_cutpurse(state, currentPlayer, handPos);
    ...
    case salvager:
     return play_salvager(state, currentPlayer, handPos, choice1);
...
int play_adventurer(struct gameState *state, int currentPlayer, int handPos, int temphand[MAX_HAND]) {
  int drawntreasure = 0;
  int z = 0;
  int cardDrawn;
  ...
  while(drawntreasure<2){
	if (state->deckCount[currentPlayer] <1){//if the deck is empty we need to shuffle discard and add to deck
      ...
      while (state->discardCount[currentPlayer] > 0) {
         state->deck[currentPlayer][state->deckCount[currentPlayer]] =
...
int play_remodel(struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2) {
...
int play_cutpurse(struct gameState *state, int currentPlayer, int handPos) {
      int i, j, k;
...
int play_salvager(struct gameState *state, int currentPlayer, int handPos, int choice1) {
  if (choice1)
    {
	 //+1 buy
     	 state->numBuys++;
