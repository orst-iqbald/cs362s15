...
int initAndCopyGameState(struct gameState *post, struct gameState *pre){
    ...
    //if NUMBER_PLAYABLE_CARDS is used instead of 16 here flip server
    //"flips out" (pun intended) when compiling so had to change it
    const int PLAYABLE_CARDS[16] = {copper, silver, gold, estate, duchy, province, adventurer, baron, council_room,gardens, great_hall, feast, mine, remodel, smithy, village};
